...
SELECT customer_id, MAX(purchase_amount) Max_Amount FROM orders GROUP BY customer_id;

-- Write an SQL statement to find the highest purchase amount on '2012-08-17' for each salesman with their ID.
SELECT salesman_id, order_date, MAX(purchase_amount) Max_Amount FROM orders 
...
SELECT customer_id, order_date, MAX(purchase_amount) Max_Amount FROM orders
